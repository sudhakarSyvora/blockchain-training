...
const { hexToBinary, generateSeedFromMnemonic } = require("./utills.js");
...
rl.question(
  "Enter entropy size (128, 160, 192, 224, 256): ",
  (entropyInput) => {
    ...
    rl.question(
      "Enter HD key derivation path (e.g., m/44'/0'/0'/0): ",
      (path) => {
        ...
        rl.question("Enter the number of accounts: ", (numAccountsInput) => {
          ...
          rl.question("Enter the passphrase(optional)", (passPhraseInput) => {
            ...
            const binaryOfHashedRandomBinaryNo =
              hexToBinary(hashedRandomBinaryNo);
            ...
            for (let i = 0; i < randomBinaryNoAndChecksum.length; i += 11) {
              seedPhrase +=
                BIP39LIST[
                  parseInt(randomBinaryNoAndChecksum.substring(i, i + 11), 2)
                ] + " ";
            }
            ...
            for (
              ...
              console.log(
                ...
                }, Public Key: ${childkey.publicExtendedKey}`
              );
            }
